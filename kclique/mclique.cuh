...
namespace graph
{
    ...
    public:
        ...
        void find_maximal_clique_node_pivot_encode_induced(COOCSRGraph_d<T>& gsplit,
            const size_t nodeOffset = 0, const size_t edgeOffset = 0)
        {
            CUDA_RUNTIME(cudaSetDevice(dev_));
            ...
            execKernel((mckernel_node_block_warp_binary_encode_induced<T, block_size, partitionSize>), grid_block_size, block_size, dev_, false,
                gsplit,
                gsplit.numNodes,
        ...
        template<const int PSIZE>
        void find_maximal_clique_node_pivot_encode_half(COOCSRGraph_d<T>& gsplit,
            const size_t nodeOffset = 0, const size_t edgeOffset = 0)
        {
            CUDA_RUNTIME(cudaSetDevice(dev_));
            const auto block_size = 128;
            ...
            auto grid_block_size = (gsplit.numNodes + block_size - 1) / block_size;
            execKernel((mckernel_node_block_warp_binary_encode_half<T, block_size, partitionSize>), grid_block_size, block_size, dev_, false,
                gsplit,
                gsplit.numNodes,
                current_level2.gdata(), cpn.gdata(),
                d_bitmap_states.gdata(), node_be.gdata(),
        ...
        template<const int PSIZE>
        void find_maximal_clique_edge_pivot_encode_induced(COOCSRGraph_d<T>& gsplit,
            const size_t nodeOffset = 0, const size_t edgeOffset = 0)
        {
            CUDA_RUNTIME(cudaSetDevice(dev_));
            const auto block_size = 128;
            ...
            execKernel((mckernel_edge_block_warp_binary_encode_induced<T, block_size, partitionSize>), grid_block_size, block_size, dev_, false,
                gsplit,
                gsplit.numEdges,
                current_level2.gdata(), cpn.gdata(),
                d_bitmap_states.gdata(), node_be.gdata(),
        ...
        template<const int PSIZE>
        void find_maximal_clique_edge_pivot_encode_half(COOCSRGraph_d<T>& gsplit,
            const size_t nodeOffset = 0, const size_t edgeOffset = 0)
        {
            CUDA_RUNTIME(cudaSetDevice(dev_));
            const auto block_size = 128;
            ...
            auto grid_block_size = (gsplit.numEdges + block_size - 1) / block_size;
            // auto grid_block_size = num_SMs * conc_blocks_per_SM;
            execKernel((mckernel_edge_block_warp_binary_encode_half<T, block_size, partitionSize>), grid_block_size, block_size, dev_, false,
                gsplit,
                gsplit.numEdges,
                current_level2.gdata(), cpn.gdata(),
                d_bitmap_states.gdata(), node_be.gdata(),
