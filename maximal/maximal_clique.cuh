...
#include "maximal_kernels.cuh"


template <typename T, uint BLOCK_DIM_X, uint CPARTSIZE>
__launch_bounds__(BLOCK_DIM_X, 16)
__global__ void
maximal_node_block_warp_pivot_count(
	uint64* counter,
...
__launch_bounds__(BLOCK_DIM_X, 16)
__global__ void
maximal_edge_block_warp_pivot_count(
	uint64* counter,
	graph::COOCSRGraph_d<T> g,
	const  graph::GraphQueue_d<T, bool>  current,
	T* current_level,
...
namespace graph
{
	...
	public:
		...
		SingleGPU_Maximal(int dev, COOCSRGraph_d<T>& g) : dev_(dev) {
		...
		SingleGPU_Maximal() : SingleGPU_Maximal(0) {}
		...
		void findMaximal_node_pivot_async(int kcount, COOCSRGraph_d<T>& g,
			ProcessingElementEnum pe, const size_t nodeOffset = 0, const size_t edgeOffset = 0)
		{
			...
			//while (todo > 0)
			{
				...
				if (current_q.count.gdata()[0] > 0)
				{
					...
					execKernel((maximal_node_block_warp_binary_pivot_count<T, block_size, partitionSize>), grid_block_size, block_size, dev_, false,
						counter.gdata(),
						g,
		...
		template<const int PSIZE>
		void findMaximal_node_nobin_pivot_async(int kcount, COOCSRGraph_d<T>& g,
			ProcessingElementEnum pe, const size_t nodeOffset = 0, const size_t edgeOffset = 0)
		{
			...
			while (todo > 0)
			{
				...
				{
					auto grid_block_size =  current_q.count.gdata()[0];
					execKernel((maximal_node_block_warp_pivot_count<T, block_size, partitionSize>), grid_block_size, block_size, dev_, false,
						counter.gdata(),
						g,
						current_q.device_queue->gdata()[0],
						current_level2.gdata(), cpn.gdata(),
		...
		////Edge
		template<const int PSIZE>
		void findMaximal_edge_pivot_async(int kcount, COOCSRGraph_d<T>& g,
			ProcessingElementEnum pe, const size_t nodeOffset = 0, const size_t edgeOffset = 0)
		{
			Log(info, "HERE\n");
			//Please be carful
			...
			const uint dv = 32;
			const uint max_level = maxDegree.gdata()[0] + 1;
			uint num_divs = (maxDegree.gdata()[0] + dv - 1) / dv;
			...
			//while (todo > 0)
			{
				...
				if (current_q.count.gdata()[0] > 0)
				{
					...
					// current_q.count.gdata()[0] = current_q.count.gdata()[0]< 80? current_q.count.gdata()[0]: 80;
					//current_q.count.gdata()[0] = 1;

						auto grid_block_size = current_q.count.gdata()[0];

						execKernel((maximal_edge_block_warp_binary_pivot_count<T, block_size, partitionSize>), grid_block_size, block_size, dev_, false,
							counter.gdata(),
							g,
							current_q.device_queue->gdata()[0],
		...
		template<const int PSIZE>
		void findMaximal_edge_nobin_pivot_async(int kcount, COOCSRGraph_d<T>& g,
			ProcessingElementEnum pe, const size_t nodeOffset = 0, const size_t edgeOffset = 0)
		{
			...
			while (todo > 0)
			{
				...
				if (current_q.count.gdata()[0] > 0)
				{
					...
					//current_q.count.gdata()[0] = 1;
						...
						execKernel((maximal_edge_block_warp_pivot_count<T, block_size, partitionSize>), grid_block_size, block_size, dev_, false,
							counter.gdata(),
							g,
							current_q.device_queue->gdata()[0],
							current_level2.gdata(), NULL,
