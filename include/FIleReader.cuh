...
private:
    ...
    */
    ...
    size_t read_bel(EdgeTy<T>* ptr, //<! buffer for edges (allocated by caller)
        ...
        if (fp_ == nullptr) {
            Log(logPriority::error, "error reading {} or file was already closed ", path_);
            return 0;
        }
        if (ptr == nullptr) {
            Log(logPriority::error, "buffer is a nullptr");
            return 0;
        }
        belBuf_.resize(24 * n);
        ...
        if (numRead != n) {
            ...
            // some error
                ...
                Log(logPriority::error, "Error while reading {}: {}", path_, strerror(errno));
                fclose(fp_);
                fp_ = nullptr;
                assert(0);
            }
            else {
                Log(logPriority::error, "Unexpected error while reading {}", path_);
                assert(0);
            }
        }
        ...
        for (; i < n; ++i) {
            ...
            if (numFilled != 3) {
                if (feof(fp_)) {
                    ...
                    Log(logPriority::critical, "Unexpected error while reading {}", path_);
                    exit(-1);
                }
            }
...
public:
    ...
    */
    EdgeListFile(const std::string& path //!< [in] the path of the file
        ...
        if (endswith(path, ".bel")) {
            ...
            Log(logPriority::critical, "no reader for file {}", path);
            exit(-1);
        }

        fp_ = fopen(path_.c_str(), "r");
        if (nullptr == fp_) {
            Log(logPriority::error, "unable to open \"{}\"", path_);
        }
    }
    ...
    */
    ...
    size_t
        ...
        default: {
            Log(logPriority::critical, "unexpected file type");
            exit(-1);
        }
        }
