...
__global__ void sgm_kernel_pre_encoded_byEdge(
    ...
    Counters *SM_times, uint64 *SM_edges,
    queue_callee(queue, tickets, head, tail))
{
    __shared__ SHARED_HANDLE<T, BLOCK_DIM_X> sh;
    __shared__ Counters times;
    T lx = threadIdx.x;
    T wx = 0;
    T partMask = 0xFFFFFFFF;

    initializeCounters<T>(&times, lx, partMask);
    ...
    while (sh.state != 100)
    {
        ...
        if (sh.state == 0)
        {
            ...
            startTime<T>(TRIANGLE, &times, lx, partMask);
            compute_triangles(sh, gh, lh);
            __syncthreads();
            endTime<T>(TRIANGLE, &times, lx, partMask);
            if (!lh.go_ahead)
                continue;
        }
        else if (sh.state == 1)
        {
            startTime<T>(WAIT, &times, lx, partMask);
            __syncthreads();
            if (threadIdx.x == 0)
            {
            ...
            __syncthreads();
            endTime<T>(WAIT, &times, lx, partMask);
            continue;
        }
        else if (sh.state == 2)
        {
        ...
        {
            startTime<T>(INITIALIZE, &times, lx, partMask);
            init_stack(sh, gh, lh, j);
            endTime<T>(INITIALIZE, &times, lx, partMask);
            if (!lh.go_ahead)
                continue;
            ...
            startTime<T>(TRAVERSE, &times, lx, partMask);
            ...
            __syncthreads();
            endTime<T>(TRAVERSE, &times, lx, partMask);
        }
        startTime<T>(QUEUING, &times, lx, partMask);
