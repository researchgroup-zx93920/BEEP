...
__global__ void get_edge_degree(graph::COOCSRGraph_d<T> g, T* edgePtr, T* maxDegree)
{
	...
	for (unsigned long long i = blockIdx.x; i < (unsigned long long) current.count[0]; i += gridDim.x)
	{
		...
		for (T k = threadIdx.x; k < num_divs_local; k += BLOCK_DIM_X )
		{
			...
			for (T sym_idx = SYMNODE_PTR[2]; sym_idx < SYMNODE_PTR[3]; sym_idx++) {
				if (SYMNODE[sym_idx] > 0) to[threadIdx.x] &= ~(level_offset[k] & get_mask(dstIdx, k));
				else to[threadIdx.x] &= orient_mask[k];
			}
		...
		if (KCCOUNT == 3)
		{
			if (threadIdx.x == 0) atomicAdd(counter, warpCount);
		...
		for (T j = wx; j < srcLen; j += numPartitions)
		{
			if ((level_offset[num_divs_local + j/32] >> (j %32)) % 2 == 0) continue;
			T* cl = level_offset + wx * (NUMDIVS * MAXLEVEL);
			...
			if (lx == 0)
			{
				if (l[wx] + 1 == KCCOUNT) sg_count[wx] += warpCount;
				else {
					l[wx]++;
					level_count[wx][l[wx] - 3] = warpCount;
					level_index[wx][l[wx] - 3] = 0;
...
template<typename T, uint BLOCK_DIM_X, uint CPARTSIZE>
__global__ void sgm_kernel_pre_encoded(
	uint64* counter,
	const graph::COOCSRGraph_d<T> g,
	const graph::GraphQueue_d<T, bool>  current,
	T* current_level,
...
)
{
	...
	for (unsigned long long i = blockIdx.x; i < (unsigned long long) current.count[0]; i += gridDim.x)
	{
		...
		if (threadIdx.x == 0)
		{
			...
			T orient_offset = offset[src] * NUMDIVS;
			orient_mask = &orient[orient_offset];

			uint64 encode_offset = (uint64) orient_offset * MAXDEG;
			encode = &adj_enc[encode_offset  /*srcStart[wx]*/];

			level_offset = &current_level[((sm_id * CBPSM) + levelPtr) * (NUMDIVS * numPartitions * MAXLEVEL)];
		}
		__syncthreads();
