...
__device__ __forceinline__ void sgm_kernel_central_node_function(
	T blockOffset,
	uint64* counter,
	const graph::COOCSRGraph_d<T> g,
...
{
	sgm_kernel_central_node_function<T, BLOCK_DIM_X, CPARTSIZE>(blockIdx.x,
...
)
{
	...
	sgm_kernel_central_node_function<T, BLOCK_DIM_X, CPARTSIZE>((sm_id * CBPSM) + levelPtr,
		counter, g, current, current_level, adj_enc, orient
	);
	__syncthreads();
